package synthesizer;

/** GuitarString.
 * @author yeahooooo.
 */
public class GuitarString {
    /** Constants. Do not change. In case you're curious.
     * the keyword final means
     * the values cannot be changed at runtime.
     * We'll discuss this and other topics
     * in lecture on Friday. */
    private static final int SR = 44100;
    /** energy decay coefficient.*/
    private static final double DECAY = .996;

    /** Buffer for storing sound data. */
    private BoundedQueue<Double> buffer;

    /** Create a guitar string of the given frequency.
     * @param frequency certain frequency.
     */
    public GuitarString(double frequency) {
        int capacity = (int) Math.round(SR / frequency);
        buffer = new ArrayRingBuffer<>(capacity);
        for (int i = 0; i < capacity; i++) {
            buffer.enqueue(0.0);
        }

    }


    /** Pluck the guitar string by replacing the buffer with white noise. */
    public void pluck() {
        while (!buffer.isEmpty()) {
            buffer.dequeue();
        }
        while (!buffer.isFull()) {
            buffer.enqueue(Math.random() - 0.5);
        }

    }

    /** Advance the simulation one time step by.
     * performing one iteration of.
     * the Karplus-Strong algorithm.
     */
    public void tic() {
        double head = buffer.dequeue();
        double newhead = buffer.peek();
        double newvalue = DECAY * (head + newhead) / 2;
        buffer.enqueue(newvalue);

    }

    /** Return the double at the front of the buffer.
     *
     * @return return buffer.peek()
     */
    public double sample() {
        return buffer.peek();
    }
}
